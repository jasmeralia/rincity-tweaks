...
## Changelog
- **2.1.14** - Fixed the shared cover-resolver's on-demand crop generation (`includes/cover-resolver.php`): Envira's `Cropping::resize_image()` silently no-ops (no error) if the source URL's domain doesn't contain `site_url()` as a substring, which a CDN-rewritten upload baseurl (e.g. CDN Enabler) fails. The resolver now rewrites the source to the real site URL before calling `envira_resize_image()`, so a genuinely missing crop can actually self-heal on a CDN-rewritten host instead of silently falling back every page load. Found while manually regenerating the 3 malformed derivatives for Odoo #401. (Odoo #402)
- **2.1.13** - Fixed the album shortcode's cover crop URL selection: the previous transformation only recognized `-scaled.<ext>` source filenames, so non-scaled sources (215 of 345 Members Gallery covers) silently fell through and emitted the full-size original instead of the existing 320x400 crop. Replaced it with a shared resolver (`includes/cover-resolver.php`) that builds the crop candidate for any source shape, validates the file exists on disk at the expected physical dimensions (catching malformed derivatives too), and generates the crop on demand via `envira_resize_image()` if missing. `rincity-gallery-favorites` now uses the same resolver. (Odoo #402)
